...
//        }
    ...
    function install_ocmod($file)
    {
        $registry = MyParcel::$registry;
        $loader = $registry->get('load');
        $loader->model('extension/modification');
        $loader->model('myparcelnl/init');
        $model_extension_modification = $registry->get('model_extension_modification');
        $model_myparcelnl_init = $registry->get('model_myparcelnl_init');
        ...
        if ($xml) {
            try {
                ...
                if ($code) {
                    ...
                    $modification_info = $model_extension_modification->getModificationByCode($code);
                ...
                if (!$json) {
                    $model_extension_modification->addModification($modification_data);
                }
        ...
        $model_myparcelnl_init->refreshOcmod();
    ...
    function uninstall_ocmod()
    {
        ...
        $results = $model_extension_modification->getModifications();
        foreach ($results as $result) {
            if ($result['code'] == 'MyParcelNL_default_checkout' || $result['code'] == 'MyParcelNL') {
                $model_extension_modification->deleteModification($result['modification_id']);
            }
        }
