...
require_once DIR_SYSTEM . 'myparcelnl/includes/class_myparcel_shipment_helper.php';
class MyParcel_View extends MyParcel_View_Core
{
    ...
    function print_batch()
    {
        ...
        if (version_compare(VERSION, '2.0.0.0', '>=')) {
            $formAction = $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'printBatch'), array('token' => $token));
        } else {
            $formAction = $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'exportPrintBatchHelper'), array('token' => $token));
        }
    ...
    function export_print_batch()
    {
        ...
        $token = $session->data['token'];
        if (version_compare(VERSION, '2.0.0.0', '>=')) {
            $formAction = $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'exportPrintBatch'), array('token' => $token));
        } else {
            $formAction = $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'exportPrintBatch'), array('token' => $token));
        }
        ob_start();
    ...
    function column_myparcel($order_id, $screen = 'order_overview')
    {
        ...
        $listing_actions = array(
            'add_shipment'		=> array (
                'url'		=> $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'shipment'), array('action' => 'add_shipment', 'token' => $token, 'order_ids[]' => $order_id)),
            ...
            'get_labels'	=> array (
                'url'		=> $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'shipment'), array('action' => 'get_labels', 'token' => $token, 'order_ids[]' => $order_id)),
            ...
            'add_return'	    => array (
                'url'		    => $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'shipment'), array('action' => 'add_return', 'token' => $token, 'order_ids[]' => $order_id)),
    ...
    **/
    ...
    function ship_to_myparcel($order_id, $return = false, $screen = 'order_overview')
    {
        ...
        if (!$return) {
            $url_myparcel_ship_to = $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'shipToMyParcel'), array('token' => $token, 'order_ids' => $order_id));
        } else {
            $url_myparcel_ship_to = $url->link(MyParcel()->getMyparcelControllerPath('myparcelnl', 'shipment'), array('token' => $token, 'action' => 'send_return'));
        }
