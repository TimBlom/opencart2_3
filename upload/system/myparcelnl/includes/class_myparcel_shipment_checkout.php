...
class MyParcel_Shipment_Checkout
{
     ...
     */
    function getTotalArray($data, $price_format = false, $order_id = null, $prefix = '')
    {
        ...
        $total_array = $this->getDeliveryTotalsFromSavedData($delivery_options, $signed, $recipient_only, $price_format, $order_id, $prefix);
        return $total_array;
    }
     ...
     */
    function getDeliveryPrices($price_format = true, $color_format = true, $prefix = '')
    {
        $registry = MyParcel::$registry;
        /** @var \Cart\Cart $cart **/
    ...
    function addDeliveryDataIntoOrder($order_info)
    {
        ...
        if ($this->checkValidShippingMethod()) {
            ...
            **/
            ...
            $current_prices = $checkout->getDeliveryPrices(true, false);
    ...
    **/
    function appliedByParcel($total)
    {
        ...
        foreach ($totals as $total_item) {
            if ($total_item['code'] == 'myparcel_total') {
                return true;
            }
        }
        return false;
    ...
    function getMyParcelShippingCost()
    {
        $registry = MyParcel::$registry;
        $config = $registry->get('config');
        $cost = $config->get('myparcel_shipping_cost');
        if (!empty($cost)) {
            return $cost;
        }
        return 0;
